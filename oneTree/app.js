...
function workerMessage(evt){
	...
	tree.position.set(TreeLocX, TreeLocY, TreeLocZ);
	twig.position.set(TreeLocX, TreeLocY, TreeLocZ);
	scene.add(tree);
	scene.add(twig);
}

this.Start=function(){
	var pos=[TreeLocX, TreeLocY, TreeLocZ];
	var gaSettings=[maxGenerations,numOfInstances,numOfChild,sameBestIter,useBest,minFitnessTerm,mutationRate,facing,trunkCollision,pos];
		//console.log("worker");
...
function init(){
	...
	*/
	...
	scene.add(twig);
	gui = new dat.GUI();
	var f0 = gui.addFolder("Tree position");
	
	var contTreeX=f0.add(self, 'TreeLocX').step(0.25);	
	contTreeX.onChange(updateTreeLoc);
	
	var contTreeY=f0.add(self, 'TreeLocY').step(0.25);	
	contTreeY.onChange(updateTreeLoc);
	
	var contTreeZ=f0.add(self, 'TreeLocZ').step(0.25);	
	contTreeZ.onChange(updateTreeLoc);
